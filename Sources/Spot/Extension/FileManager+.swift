...
extension Suffix where Base: FileManager {
	...
	func permissionsForItem(at URL: URL) throws -> UInt16 {
		let entryFileSystemRepresentation = base.fileSystemRepresentation(withPath: URL.path)
		var fileStat = stat()
	...
	func fileSizeForItem(at url: URL) throws -> UInt32 {
		guard base.fileExists(atPath: url.path) else {
			throw AttributedError(.fileNotFound, userInfo: [NSFilePathErrorKey: url.path])
		}
		let representation = base.fileSystemRepresentation(withPath: url.path)
		var _stat = stat()
